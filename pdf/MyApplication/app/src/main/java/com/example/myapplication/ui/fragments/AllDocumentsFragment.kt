...
class AllDocumentsFragment : BaseDocumentFragment() {
    ...
    private fun setupRecyclerView() {
        binding.recyclerView.apply {
            ...
            // Add scroll listener to control animation state
            addOnScrollListener(object : androidx.recyclerview.widget.RecyclerView.OnScrollListener() {
                override fun onScrollStateChanged(recyclerView: androidx.recyclerview.widget.RecyclerView, newState: Int) {
                    super.onScrollStateChanged(recyclerView, newState)
                    when (newState) {
                        androidx.recyclerview.widget.RecyclerView.SCROLL_STATE_DRAGGING -> {
                            // Start animations when scrolling starts
                            (adapter as? DocumentAdapter)?.setScrolling(true)
                        }
                        androidx.recyclerview.widget.RecyclerView.SCROLL_STATE_IDLE -> {
                            // Stop animations when scrolling stops
                            (adapter as? DocumentAdapter)?.setScrolling(false)
                        }
                    }
                }
                ...
                override fun onScrolled(recyclerView: androidx.recyclerview.widget.RecyclerView, dx: Int, dy: Int) {
                    ...
                    // Only apply animations when scrolling down (dy > 0)
                    if (dy > 0) {
                        (adapter as? DocumentAdapter)?.setScrolling(true)
                    } else {
                        (adapter as? DocumentAdapter)?.setScrolling(false)
    ...
    private fun applyFilters() {
        ...
        binding.noDataText.visibility = if (finalDocs.isEmpty()) View.VISIBLE else View.GONE
    }
