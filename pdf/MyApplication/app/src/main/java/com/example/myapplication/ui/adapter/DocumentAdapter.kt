...
class DocumentAdapter(
    ...
    override fun onBindViewHolder(holder: DocumentViewHolder, position: Int) {
        ...
        // Apply animation only when scrolling
        if (isScrolling) {
            val animation = if (position == documents.size - 1) {
                // Use bounce animation for the last item
                AnimationUtils.loadAnimation(holder.itemView.context, R.anim.item_bounce_animation)
            } else {
                // Use normal scroll animation for other items
                AnimationUtils.loadAnimation(holder.itemView.context, R.anim.item_scroll_animation)
            }
